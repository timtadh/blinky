...
public class TraceEvent implements MethodDecl, Instruction {
  ...
  /**
   * Creates a trace event that can store properties for the execution of a static instruction, and
   * its properties are specified with {@link InsnExecPropNames}.
   *
  ...
  /**
   * Creates a trace event that can store properties for a static instruction in a method
   * definition, as specified by {@link InsnPropNames}.
   *
   * @param id Unique id for the event.
  ...
  /**
   * Creates a trace event that can store properties for a method or class declaration, as specified
   * by {@link DeclPropNames}.
   *
   * @param id Unique id for the event.
   * @param type The type of this event (can be null).
   * @return
  ...
  public static TraceEvent valueOf(String eventString) {
    ...
    MyAssert.assertThat(
        event.getPropCount() <= (split.length - offset),
  ...
  public String getProp(final Enum propName) {
    if (!enumChecksOut(propName)) return null;
  ...
  public void setProp(final Enum propName, final String propValue) {
    if (enumChecksOut(propName)) setProp(propName.ordinal(), propValue);
  }

  private boolean enumChecksOut(final Enum propName) {
    if (propName == null || propName.getDeclaringClass() != propNames[0].getDeclaringClass())
      return false;
    return true;
  }
