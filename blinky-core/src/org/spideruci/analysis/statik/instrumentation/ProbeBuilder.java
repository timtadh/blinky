...
/**
 * This class presents an abstraction to create bytecode instruction sequences pertaining to the
 * instrumentation probes that are meant to call the Profiler code when such probes are executed
 * with the rest of the program during execution.
 *
 * @author vpalepu
 */
public class ProbeBuilder implements Opcodes {
    ...
    if (opcode == Opcodes.PUTFIELD) { // visiting non-static field dereferences.
      ...
      if (isRef) { // ref, val
        ...
        if (isWide) { // ref, val1, val2
          ...
          //          visitLoadHash();
          mv.visitMethodInsn(
              INVOKESTATIC, PROFILER_NAME, GETHASH, GETHASH_DESC, false); // ref, val, ref#
          //          mv.visitLdcInsn("0");
          mv.visitLdcInsn("0"); // ref, val, ref#, "0"
        }
      }
  ...
  public ProbeBuilder passArrayLoadStackArgs(int opcode) {
    MyAssert.assertThat(Opcodes.IALOAD <= opcode && opcode <= Opcodes.SALOAD, "not an array load.");
  ...
  public ProbeBuilder passArrayStoreStackArgs(int opcode) {
    MyAssert.assertThat(
        Opcodes.IASTORE <= opcode && opcode <= Opcodes.SASTORE, "not an array store.");
    ...
    switch (opcode) {
      ...
      case Opcodes.FASTORE:
        mv.visitMethodInsn(
            Opcodes.INVOKESTATIC, "java/lang/String", "valueOf", "(F)Ljava/lang/String;", false);
        break;
      ...
      case Opcodes.LASTORE:
        mv.visitMethodInsn(
            Opcodes.INVOKESTATIC, "java/lang/String", "valueOf", "(J)Ljava/lang/String;", false);
        break;
      case Opcodes.DASTORE:
        mv.visitMethodInsn(
            Opcodes.INVOKESTATIC, "java/lang/String", "valueOf", "(D)Ljava/lang/String;", false);
        break;
      default:
  ...
  public void build(String callBackName, String classbackClassName, String returnType) {
    ...
    this.mv.visitMethodInsn(
        Opcodes.INVOKESTATIC, classbackClassName, callBackName, callbackDesc.toString(), false);
  }
