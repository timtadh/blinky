...
public class BytecodeMethodAdapter extends AdviceAdapter {
  ...
  public BytecodeMethodAdapter(
      MethodDecl methodDecl, int access, String name, String desc, MethodVisitor mv) {
  ...
  protected void onMethodEnter() {
      ...
      for (int i = 0; i < argTypes.length; i += 1) {
        ...
        if (argInitial == 'L') {
          ...
          mv.visitMethodInsn(
              ...
              Profiler.GETTYPENAME,
              Profiler.GETTYPENAME_DESC,
              false);
        } else {
          mv.visitLdcInsn(argType);
        }
    ...
    String instructionLog =
        buildInstructionLog(-1, -1, EventType.$enter$, opcode, methodDecl.getId());
  ...
  protected void onMethodExit(int opcode) {
    ...
    EventType eventType = (opcode == Opcodes.ATHROW) ? EventType.$athrow$ : EventType.$return$;
    String instructionLog =
        buildInstructionLog(byteIndex, lineNum, eventType, opcode, methodDecl.getId());
  ...
  public void visitLineNumber(int line, Label start) {
    ...
    if (shouldInstrument && Profiler.logSourceLineNumber) {
      final int opcode = -1;
      String instructionLog =
          buildInstructionLog(-1, line, EventType.$line$, opcode, methodDecl.getId());
  ...
  public void visitMethodInsn(int opcode, String owner, String name, String desc) {
    ...
    if (Profiler.logMethodInvoke) {
      ...
      if (Profiler.logInvokeRuntimeSign) {
        ...
        InvokeSignCallBack.buildArgProfileProbe(
            mv,
            opcode,
            owner,
            name,
      ...
      instructionLog =
          buildInstructionLog(
              byteIndex, lineNum, EventType.$complete$, -4, methodDecl.getId(), owner, name, desc);
  ...
  public void visitFieldInsn(int opcode, String owner, String name, String desc) {
    ...
    if (Profiler.logField) {
      ...
      String instructionLog =
          buildInstructionLog(
              byteIndex, lineNum, EventType.$field$, opcode, methodDecl.getId(), owner, name, desc);
  ...
  public void visitJumpInsn(int opcode, Label label) {
    if (shouldInstrument && Profiler.logJump) {
      ...
      String instructionLog =
          buildInstructionLog(byteIndex, lineNum, EventType.$jump$, opcode, methodDecl.getId());
  ...
  public void visitLdcInsn(Object cst) {
    if (cst != null && cst instanceof String && ((String) cst).startsWith(BYTECODE_LDC_MARKER)) {
  ...
  private void onConstantInsn(int opcode, String value) {
    if (shouldInstrument && Profiler.logConstant) {
      ...
      String instructionLog =
          buildInstructionLog(
              byteIndex, lineNum, EventType.$constant$, opcode, methodDecl.getId(), value);
  ...
  private void onSwtich(int opcode) {
    if (shouldInstrument && Profiler.logSwitch) {
      ...
      String instructionLog =
          buildInstructionLog(byteIndex, lineNum, EventType.$switch$, opcode, methodDecl.getId());
